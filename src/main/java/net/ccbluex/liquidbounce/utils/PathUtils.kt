...
object PathUtils : MinecraftInstance() {
                ...
                if (pathElm!!.squareDistanceTo(lastDashLoc) > dashDistance * dashDistance) {
                    ...
                    cordsLoop@ while (x <= bigX) {
                        ...
                        while (y <= bigY) {
                            var z = smallZ.toInt()
                            while (z <= bigZ) {
                                if (!AStarCustomPathFinder.Companion.checkPositionValidity(x, y, z, false)) {
                                    canContinue = false
                                    break@cordsLoop
                                }
                                z++
    ...
    fun findPath(tpX: Double, tpY: Double, tpZ: Double, offset: Double): List<Vector3d> {
        ...
        var d = 1.0
        while (d <= steps) {
            positions.add(Vector3d(mc.thePlayer.posX + dX * d / steps, mc.thePlayer.posY + dY * d / steps, mc.thePlayer.posZ + dZ * d / steps))
            ++d
        }
        return positions
    }
