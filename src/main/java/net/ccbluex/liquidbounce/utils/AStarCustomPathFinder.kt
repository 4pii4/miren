...
import net.ccbluex.liquidbounce.utils.block.BlockUtils.floorVec3
...
class AStarCustomPathFinder(startVec3: Vec3, endVec3: Vec3) {
    ...
    fun addHub(parent: Hub?, loc: Vec3, cost: Double): Boolean {
        ...
        if (existingHub == null) {
            if (loc.xCoord == endVec3.xCoord && loc.yCoord == endVec3.yCoord && loc.zCoord == endVec3.zCoord || minDistanceSquared != 0.0 && loc.squareDistanceTo(endVec3) <= minDistanceSquared) {
                ...
                val path: ArrayList<Vec3> = ArrayList(parent!!.path)
                path.add(loc)
    ...
    inner class Hub(loc: Vec3?, parent: Hub?, path: ArrayList<Vec3>, squareDistanceToFromTarget: Double, cost: Double, totalCost: Double) {
        var loc: Vec3? = null
        var parent: Hub? = null
        var path: ArrayList<Vec3>
