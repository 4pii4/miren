...
object StringUtils {
    ...
    fun replace(string: String, searchChars: String, replaceChars: String?, forceReload: Boolean = false): String {
        ...
        if (!forceReload && stringRegexCache[searchChars] != null && stringRegexCache[searchChars] == replaceChars && stringReplaceCache.containsKey(string)) return stringReplaceCache.getOrDefault(string, replace(string, searchChars, replaceChars, true)) // will attempt to retry replacement once again
