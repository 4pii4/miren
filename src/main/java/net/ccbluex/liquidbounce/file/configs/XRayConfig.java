...
public class XRayConfig extends FileConfig {
    ...
    protected void loadConfig() throws IOException {
        ...
        if(xRay == null) {
            ClientUtils.logger.error("[FileManager] Failed to find xray module.");
            return;
        }
        ...
        for(final JsonElement jsonElement : jsonArray) {
            try {
                ...
                if (xRay.getXrayBlocks().contains(block)) {
                    ClientUtils.logger.error("[FileManager] Skipped xray block '" + block.getRegistryName() + "' because the block is already added.");
                    continue;
                }
