...
class BufferSpeed : Module() {
    ...
    private val stairsBoostValue = FloatValue("StairsBoost", 1.87f, 1f, 2f) { stairsValue.get() }
    private val stairsModeValue = ListValue("StairsMode", arrayOf("Old", "New"), "New") { stairsValue.get() }
    private val slabsValue = BoolValue("Slabs", true)
    private val slabsBoostValue = FloatValue("SlabsBoost", 1.87f, 1f, 2f) { slabsValue.get() }
    private val slabsModeValue = ListValue("SlabsMode", arrayOf("Old", "New"), "New") { slabsValue.get() }
    private val iceValue = BoolValue("Ice", false)
    private val iceBoostValue = FloatValue("IceBoost", 1.342f, 1f, 2f) { iceValue.get() }
    private val snowValue = BoolValue("Snow", true)
    private val snowBoostValue = FloatValue("SnowBoost", 1.87f, 1f, 2f) { snowValue.get() }
    private val snowPortValue = BoolValue("SnowPort", true)
    private val wallValue = BoolValue("Wall", true)
    private val wallBoostValue = FloatValue("WallBoost", 1.87f, 1f, 2f) { wallValue.get() }
    private val wallModeValue = ListValue("WallMode", arrayOf("Old", "New"), "New") { wallValue.get() }
    private val headBlockValue = BoolValue("HeadBlock", true)
    private val headBlockBoostValue = FloatValue("HeadBlockBoost", 1.87f, 1f, 2f) { headBlockValue.get() }
    ...
    private val isNearBlock: Boolean
        get() {
            ...
            for (blockPos in blocks) if (theWorld.getBlockState(blockPos).block.blockBoundsMaxY ==
                ...
                theWorld.getBlockState(blockPos).block === Blocks.barrier
            ) return true
            return false
        }
}