...
class TargetMark : Module() {
    ...
    fun onTick(event: TickEvent?) {
        if (modeValue.get().equals("jello", ignoreCase = true) && aura!!.targetModeValue.get() != "Multi") al = AnimationUtils.changer(al, if (aura!!.target != null) jelloFadeSpeedValue.get() else -jelloFadeSpeedValue.get(), 0f, colorAlphaValue.get() / 255.0f)
    }

    @EventTarget
    fun onRender3D(event: Render3DEvent?) {
        if (modeValue.get().equals("jello", ignoreCase = true) && aura!!.targetModeValue.get() != "Multi") {
            val lastY = yPos
            if (al > 0f) {
            ...
            if (aura!!.targetModeValue.get() != "Multi" && aura!!.target != null) RenderUtils.drawPlatform(aura!!.currentTarget, if (aura!!.currentTarget!!.hurtTime > 0) reAlpha(getColor(aura!!.target), colorAlphaValue.get()) else Color(235, 40, 40, colorAlphaValue.get()))
        } else if (modeValue.get().equals("tracers", ignoreCase = true)) {
            if (aura!!.targetModeValue.get() != "Multi" && aura!!.target != null) {
            ...
            if (aura!!.targetModeValue.get() != "Multi" && aura!!.target != null) RenderUtils.drawEntityBox(aura!!.target, if (aura!!.target!!.hurtTime > 3) reAlpha(getColor(aura!!.target), colorAlphaValue.get()) else Color(255, 0, 0, colorAlphaValue.get()), false)
        }
    }
