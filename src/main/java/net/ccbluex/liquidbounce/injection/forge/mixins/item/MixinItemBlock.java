        ...
        if (stack.stackSize == 0) {
            ...
            if (worldIn.setBlockState(pos, iblockstate1, 3)) {
                ...
                if (ViaLoadingBase.getInstance().getTargetVersion().getOriginalVersion() != ViaMCP.NATIVE_VERSION) {
                    ...
                    mc.theWorld.playSoundAtPos(pos.add(0.5, 0.5, 0.5), block.stepSound.getPlaceSound(), block.stepSound.getVolume() + 1.0F / 2.0F, block.stepSound.getFrequency() * 0.8F, false);
                } else {
                    worldIn.playSoundEffect((float) pos.getX() + 0.5F, (float) pos.getY() + 0.5F, (float) pos.getZ() + 0.5F, block.stepSound.getPlaceSound(), (block.stepSound.getVolume() + 1.0F) / 2.0F, block.stepSound.getFrequency() * 0.8F);
                }
